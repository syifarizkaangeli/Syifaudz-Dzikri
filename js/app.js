(function(){
  ...
  function iconPause(){
    return `<svg width="16" height="16" viewBox="0 0 24 24" fill="white"><rect x="6" y="5" width="4" height="14"/><rect x="14" y="5" width="4" height="14"/></svg>`;
  }
  ...
  function renderHome(){
    ...
    headerExtra.innerHTML = `
      <div class="brand-tagline">
        Menjadikan ayat-ayat Al-Qur'an sebagai obat untuk selalu mengingat Allah SWT akan kebesaran-Nya.
      </div>
    `;

    view.innerHTML = `
      <div class="tab-bar">
        <button class="tab-btn ${homeTab==='quran'?'active':''}" data-tab="quran">
          Al-Qur'an
        </button>

        <button class="tab-btn ${homeTab==='doa'?'active':''}" data-tab="doa">
          Kumpulan Doa
        </button>
      </div>
  ...
  function renderQuranTab(){
      ...
      <div class="search-wrap">
        ...
        <div class="search-box">
          ...
          <input
            id="searchInput"
            type="text"
            placeholder="Cari surah, kata dalam ayat, atau 'Al-Baqarah 255'..."
          />

          <button
            class="icon-btn"
            id="micBtn"
            title="Cari pakai suara"
          >
            ${iconMic()}
          </button>
        </div>
        ...
        <div class="mic-hint" id="micHint">
          Tekan mikrofon lalu ucapkan nama surah atau kata yang dicari.
        </div>

      </div>

      <div class="home-quran-action">
        <button class="play-all-btn" id="playAllQuranBtn">
          ${iconPlay()} Putar Semua Surah
        </button>
      </div>
    ...
    if(!surahList.length){
      surahList = SURAH_DATA;
    }

    renderSurahList(surahList);

    document
      .getElementById('playAllQuranBtn')
      .addEventListener('click', togglePlayAllQuran);
    ...
    document
      ...
      .addEventListener('input', (e)=>{
        ...
        // Cek pola "nomor:ayat" atau "nomor ayat"
        ...
        // Cek pola "nama surah" + "nomor ayat"
        const words = q.split(/\s+/);

        if(
          words.length >= 2 &&
          /^\d{1,3}$/.test(words[words.length - 1])
        ){
        ...
        // Cari isi ayat lewat API
        if(q.length >= 3){

          ayatSearchTimer = setTimeout(
            ()=> searchAyatContent(raw),
            450
          );
  ...
  function showJumpCard(sMeta, aNum){
    ...
    document.getElementById('ayatSearchWrap').innerHTML =
      ...
      <div class="surah-list">
        ...
        <button class="surah-card" id="jumpBtn">
          ...
          <div class="surah-info">
            ...
            <div class="latin">
              ${esc(sMeta.englishName)} — Ayat ${aNum}
            </div>
  ...
  function searchAyatContent(query){
    ...
    fetch(
      `${API}/search/${encodeURIComponent(query)}/all/id.indonesian`
    )
      .then(r=>r.json())
      ...
      .then(d=>{
        ...
        wrap.innerHTML = `
          <div class="section-label">
            Hasil Pencarian Ayat (${matches.length})
          </div>
          ...
          <div class="surah-list">
            ...
            ${matches.slice(0,30).map(m=>`
              ...
              <button
                class="surah-card"
                data-snum="${m.surah.number}"
                data-anum="${m.numberInSurah}"
              >
        ...
        wrap
          ...
          .forEach(btn=>{
            ...
            btn.addEventListener(
              'click',
              ()=> renderSurahDetail(
                parseInt(btn.dataset.snum,10),
                parseInt(btn.dataset.anum,10)
              )
            );
  ...
  function renderSurahList(list){
    ...
    const el =
      document.getElementById('surahListEl');
  ...
  function setupMic(){
    ...
    const micHint =
      document.getElementById('micHint');
    ...
    if(!SpeechRecognition){
      ...
      micBtn.addEventListener(
        ...
        ()=>{
          micHint.textContent =
            "Pencarian suara tidak didukung di browser ini. Gunakan Google Chrome di Android.";
        }
      );
    ...
    micBtn.addEventListener(
      ...
      ()=>{
        ...
        try{
          ...
          micHint.textContent =
            "Mendengarkan… silakan sebutkan nama surah.";
    ...
    rec.onerror = (event)=>{
      ...
      let msg =
        "Tidak dapat menangkap suara. Coba lagi.";
      ...
      if(
        ...
        msg =
          "Mikrofon belum diizinkan. Ketuk ikon gembok/info di address bar, lalu izinkan akses Mikrofon untuk halaman ini.";

      }else if(event.error === 'network'){

        msg =
          "Pencarian suara butuh koneksi internet. Periksa sinyal/WiFi lalu coba lagi.";

      }else if(event.error === 'no-speech'){

        msg =
          "Suara tidak terdengar. Coba ucapkan lebih dekat ke mikrofon.";
  ...
  async function fetchSurahData(num){
    ...
    if(cache[num] && cache[num].base){
      return cache[num];
    }


    const editions =
      "quran-uthmani,en.transliteration,id.indonesian,ar.alafasy";


    const r =
      await fetch(
        `${API}/surah/${num}/editions/${editions}`
      );
    ...
    const trans =
      data.find(
        e=>e.edition.identifier==='id.indonesian'
      );

    const audio =
      data.find(
        e=>e.edition.identifier==='ar.alafasy'
      );
  ...
  function wordButtonsForAyah(
    surahNum,
    ayah
  ){

    const words =
      splitArabicWords(ayah.arabic);
  ...
  function playSingleWord(
    ...
    document
      .querySelectorAll('.ayat-play.playing')
      .forEach(
    ...
    document
      .querySelectorAll('.ayat-card.active')
      .forEach(
    ...
    document
      .querySelectorAll('.arabic-word.playing')
      .forEach(
        el=>el.classList.remove('playing')
      );
  ...
  async function togglePlayAllQuran(){
    ...
    if(btn){
      ...
      btn.innerHTML =
        `${iconPause()} Hentikan Putar Semua Surah`;
  ...
  async function playNextQuranAyah(){
    ...
    const meta =
      SURAH_DATA[quranSurahIndex];


    try{

      const data =
        await fetchSurahData(meta.number);
      ...
      setMiniPlayer(
        true,
        `${meta.englishName} • Ayat ${a.number}`
      );


      setMediaSession(
        meta.englishName,
        `Ayat ${a.number} dari ${data.ayahs.length}`
      );
  ...
  function renderSurahDetail(
    ...
    view.innerHTML = `
      ...
      >
        ...
        <div
          style="font-family:'Nunito',sans-serif; font-weight:800; font-size:20px; color:var(--emerald-800);"
        >
          ${meta ? esc(meta.englishName) : 'Surah'}

          <span
            style="color:var(--ink-soft); font-weight:600; font-size:14px;"
          >
            (${meta ? meta.englishNameTranslation : ''})
          </span>
        ...
        <div
          style="font-family:'Amiri',serif; font-size:26px; color:var(--emerald-700); margin-top:4px;"
        >
          ${meta ? meta.name : ''}
        </div>
  ...
  function buildAyatUI(
      ...
      ayahs.map(a=>`
        ...
        >
          ...
          <div class="ayat-arabic">
            ${wordButtonsForAyah(num, a)}
          </div>


          <div class="word-hint">
            Ketuk satu kata untuk mendengarkan kata itu saja.
          </div>
        ...
        btn.addEventListener(
          ...
          ()=>{
            ...
            playSingleWord(
              ...
              parseInt(
                btn.dataset.wordSurah,
                10
              ),

              parseInt(
                btn.dataset.wordAyah,
                10
              ),

              parseInt(
                btn.dataset.wordIndex,
                10
              )
    ...
    if(scrollToAyah){
      ...
      const target =
        c.querySelector(
          `.ayat-card[data-ayah="${scrollToAyah}"]`
        );
  ...
  function togglePlayAll(){
    ...
    playQueue =
      cache[currentSurah].ayahs.slice();
    ...
    btn.innerHTML =
      `${iconPause()} Hentikan Putar Surah`;
  ...
  function playQueueStep(){
    ...
    setMiniPlayer(
      true,
      `Ayat ${a.number} / ${playQueue.length}`
    );


    setMediaSession(
      surahLabel(currentSurah),
      `Ayat ${a.number} dari ${playQueue.length}`
    );
  ...
  function renderDoaTab(){
    ...
    c.innerHTML = `
      ...
      <div class="search-wrap">
        ...
        <div class="search-box">
          ...
          <input
            id="doaSearchInput"
            type="text"
            placeholder="Cari doa, mis. 'hutang' atau 'kesedihan'..."
          />
      ...
      <div class="doa-filter-row">
        ...
        <button
          class="chip ${doaShowFavOnly?'':'active'}"
          data-fav="0"
        >
          Semua Doa
        </button>
    ...
    if(!items.length){
      ...
      wrap.innerHTML = `
        ...
        <div class="empty-msg">
          ...
          ${
            doaShowFavOnly
              ? 'Belum ada doa favorit. Ketuk ikon hati pada doa untuk menyimpannya.'
      ...
      <div class="doa-detail">
        ...
        <div class="doa-detail-top">
          ...
          <div>
            ...
            <div class="doa-detail-cat">
              ${esc(doa.category)}
            </div>

            <div class="doa-detail-title">
              ${esc(doa.title)}
            </div>
  ...
  // bikin app installable & app-shell-nya kebuka lebih cepat
